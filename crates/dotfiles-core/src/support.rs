...
pub use runtime_facts::{
    detect_full_xcode, render_runtime_facts, write_runtime_facts, RUNTIME_FACTS_FILE,
};
...
mod inputs {
    ...
    mod tests {
        ...
        fn resolve_target_uses_targets_by_profile_for_explicit_profile() {
            ...
            let resolved =
                resolve_target_from_manifest(&manifest, "work_mac", Some("lite")).expect("resolve");
...
mod runtime_facts {
    use super::process::run_command_output;
    use super::targets_manifest::nix_string;
    use std::fs;
    use std::path::{Path, PathBuf};
    ...
    pub const RUNTIME_FACTS_FILE: &str = "runtime.nix";

    pub fn detect_full_xcode() -> bool {
        ...
        xcode_select.arg("-p");
        let developer_dir = match run_command_output(&mut xcode_select) {
        ...
        if !developer_dir.contains(".app/Contents/Developer") {
            return false;
        }

        let xcodebuild = PathBuf::from(&developer_dir)
            .join("usr")
            .join("bin")
            .join("xcodebuild");
        ...
        let mut xcodebuild_version = Command::new(xcodebuild);
        xcodebuild_version.arg("-version");
        matches!(
            run_command_output(&mut xcodebuild_version),
            Ok(output) if output.status.success()
        )
    }

    pub fn render_runtime_facts(host: &str, full_xcode: bool) -> String {
        format!(
            r#"# Generated by dotfiles apply. Do not edit.
{{
  machines = {{
    {} = {{
      extra.fullXcode = {};
...
"#,
            nix_string(host),
            if full_xcode { "true" } else { "false" },
        )
    }

    pub fn write_runtime_facts(facts_dir: &Path, host: &str) -> Result<bool, String> {
        let full_xcode = detect_full_xcode();
        let runtime_facts_path = facts_dir.join(RUNTIME_FACTS_FILE);
        fs::write(&runtime_facts_path, render_runtime_facts(host, full_xcode))
            .map_err(|err| format!("failed to write {}: {}", runtime_facts_path.display(), err))?;

        #[cfg(unix)]
        fs::set_permissions(&runtime_facts_path, fs::Permissions::from_mode(0o600))
            .map_err(|err| format!("failed to chmod {}: {}", runtime_facts_path.display(), err))?;
    ...
    #[cfg(test)]
    mod tests {
        use super::render_runtime_facts;

        #[test]
        fn render_runtime_facts_quotes_host_key() {
            let rendered = render_runtime_facts("own_mac", true);
            assert!(rendered.contains("\"own_mac\" = {"));
            assert!(rendered.contains("extra.fullXcode = true;"));
        }

        #[test]
        fn render_runtime_facts_escapes_host_key() {
            let rendered = render_runtime_facts("host\"name", false);
            assert!(rendered.contains("\"host\\\"name\" = {"));
            assert!(rendered.contains("extra.fullXcode = false;"));
        }
    }
}
