...
- Fish immutable layer:
  - `~/.config/fish/hm-fish/config.fish` (generated by Home Manager; sourced from the writable runtime wrapper)
...
- Fish support is Home Manager/runtime only in this repo. Managed macOS login-shell switching still supports `zsh` and `bash`.
