...
- `sync emacs --apply --bootstrap` は先に config を書き、Doom ではない `${EMACSDIR:-~/.emacs.d}` を `.pre-doom.YYYYmmddHHMMSS` に退避してから Doom を shallow clone し、`doom install` を実行する。既存 Doom checkout では `doom sync` を実行する
...
- Model:
  ...
  - `tools.system.karabiner` のような feature module は settings policy を所有し、`tools.editor.emacs` と `tools.editor.vscode` は repo-managed editor state と sync tooling を所有する
