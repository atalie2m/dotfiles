...
- `tools.editor.emacs.enable` は legacy `~/.emacs.d` redirect shim も所有し、GUI Emacs が `${EMACSDIR:-~/.config/emacs}` から Doom を読むようにする
- `tools.editor.emacs.bootstrap.enable` は `${EMACSDIR:-~/.config/emacs}/bin/doom` が無い場合だけ `dotfiles-doom bootstrap` を実行する
