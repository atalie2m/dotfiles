...
このリポジトリは、Nix flakes 経由で使える Web 開発テンプレートを公開しています。
...
- Node.js 22、pnpm、bun、AWS CLI v2、jq/yq、mkcert、just を含む dev shell（`wrangler` は任意で、default ではコメントアウト）
- treefmt-nix 経由で統合された Prettier formatting と `apps.format`
- 開発タスク用の `nix run .#dev` と `nix flake check` hook
...
運用上の注意: サポートされる root flake API は Darwin-first で、`darwinConfigurations` と `templates.web-dev` を公開します。
...
- `sync vscode` は Rust engine（`dotfiles-sync-vscode`）を使う
...
shell、VS Code、system app surface の mutable / immutable boundary は [`docs/reconciled-surfaces.md`](reconciled-surfaces.md) を参照してください。
...
default の Zsh prompt は Pure です。`base` はこの prompt-only setup を維持し、`dev` / `pro` / `ultra` では次も有効になります。
...
- `HOME` は `sync shell`、`sync vscode`、および default user-scoped path が必要な command で必須
...
- **`HOME is not set`** → `sync shell`、`sync vscode`、および default user-scoped runtime path は `HOME` が必要です。`HOME` を export するか、[`docs/commands.md`](commands.md#runtime-overrides) にある明示 override を使ってください。
