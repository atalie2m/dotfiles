...
- platform 間で再利用する portable profile bundle の `nix/catalog/shared/{bundles.nix}`
...
運用上の注意: サポートされる root flake API は Darwin-first で、共有 development workbench 向けの bounded Linux Home Manager target も公開します。Linux target が所有するのは LXC 内の interactive userland のみです。LXC substrate、storage、Tailscale、SSH、observability、lifecycle は `domus-ops` が引き続き所有します。
...
Linux workbench target は username、home directory、machine values を local facts
から `myconfig.hostContext` 経由で導出します。target key は live LXC hostname と
一致する必要はありません。`nix build .#homeConfigurations.linux_workbench.activationPackage`
で build し、LXC 上では `home-manager switch --flake .#linux_workbench` で switch
します。Codex CLI はこの profile では管理しません。switch 後に `codex --version`
で standalone upstream installer が live host 上で優先されていることを確認します。
...
2. stock host profile と host override は `go`, `nodejs`, `opentofu`, `terraform` の global opt-in toggle を提供しません。machine-wide version が repo 間に漏れないよう、project template/devShell 側に閉じます。
