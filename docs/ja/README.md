...
- Neovim の install と config setup は分離されている。`tools.editor.neovim.enable` は Neovim を install し、`tools.editor.neovim.sync.enable` は `apps/neovim/` の repo-managed LazyVim config を wire する。`ultra` profile は setup を有効化し、`pro` は editor だけを install する
