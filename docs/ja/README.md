...
- Emacs の app wiring は Nix-first だが、Doom config file と package state は mutable のまま扱う。`sync emacs` は `apps/emacs/doom/{init,packages,config}.el` と `~/.config/doom` 配下の writable file を reconcile し、`--adopt` で local Doom edit を repo に取り込む。Doom 本体は `${EMACSDIR:-~/.emacs.d}` に置くため、標準の GUI / daemon 起動でも追加引数なしで読み込まれる。`ultra` profile は activation-time Emacs sync と初回 Doom bootstrap を実行し、`pro` は Emacs を install するだけで setup は行わない
