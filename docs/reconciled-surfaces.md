...
- Fish immutable layer:
  - `~/.config/fish/hm-fish/config.fish` (generated by Home Manager; not managed by runtime sync)
- State: none
...
- Fish support is Home Manager/runtime only in this repo. Managed macOS login-shell switching still supports zsh/bash.
