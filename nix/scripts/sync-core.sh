...
sync_core_init_cli_defaults() {
  sync_core_mode="check"
  sync_core_details=0
  sync_core_show_diff=0
  sync_core_in_place=0
  sync_core_force=0
  sync_core_output_dir=""
  sync_core_item_filter=""
}
sync_core_is_selected_default() {
  local id="$1"
  if [[ -n ${sync_core_item_filter:-} && $id != "$sync_core_item_filter" ]]; then
    return 1
  fi
  return 0
}
sync_core_is_selected() {
  local id="$1"
  local desired_meta="$2"
  local actual_meta="$3"
  if ! sync_core_is_selected_default "$id"; then
    return 1
  fi
  if _sync_core_has_function sync_adapter_is_selected; then
    sync_adapter_is_selected "$id" "$desired_meta" "$actual_meta"
    return $?
  fi
  return 0
}
sync_core_on_no_selection() {
  if _sync_core_has_function sync_adapter_on_no_selection; then
  ...
  die "no sync items selected"
}
sync_core_resolve_surface_dir() {
  local required_path="$1"
  local current_dir="$2"
  local is_default_flag="$3"
  local resolved="$current_dir"
  local worktree_root=""
  if [[ $is_default_flag -eq 1 ]]; then
    worktree_root="$(resolve_repo_worktree_root_for "$required_path" || true)"
    if [[ -n $worktree_root ]]; then
      sync_core_root="$worktree_root"
      resolved="$sync_core_root/$required_path"
    fi
  fi
  printf '%s\n' "$resolved"
}
sync_core_parse_cli_args() {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    --apply)
      ...
      sync_core_mode="apply"
      shift
      ;;
    --adopt)
      sync_core_mode="adopt"
      shift
      ;;
    --forget)
      sync_core_mode="forget"
      shift
      ;;
    --details)
      sync_core_details=1
      ...
      ;;
    --diff)
      sync_core_show_diff=1
      shift
      ;;
    --in-place)
      sync_core_in_place=1
      shift
      ;;
    --force)
      sync_core_force=1
...
sync_core_state_key_for_id() {
  local id="$1"
  if _sync_core_has_function sync_adapter_state_key; then
    sync_adapter_state_key "$id"
    return 0
  fi
  printf '%s\n' "$id"
}
...
sync_core_read_last_applied_hash() {
  ...
  key="$(sync_core_state_key_for_id "$id")"
...
sync_core_log_status() {
  ...
  case "$status" in
  safe-update)
    log "pending update: $id"
    log "  reason: desired changed while actual still matches lastApplied"
    ;;
  in-sync-untracked)
    log "in-sync (untracked): $id"
    ;;
  state-stale)
    log "in-sync (state stale): $id"
    ;;
  missing)
    log "missing actual content: $id"
    ;;
  drift-untracked | drift-missing | drift-external | conflict)
    ...
    case "$status" in
    drift-untracked)
      log "  reason: actual differs from desired without lastApplied state"
      ;;
    drift-missing)
      log "  reason: actual is missing while lastApplied exists"
      ;;
    drift-external)
      log "  reason: actual changed outside dotfiles"
      ;;
    conflict)
      log "  reason: desired and actual both changed from lastApplied"
      ;;
    esac
    ;;
  *)
...
sync_core_run_forget() {
  ...
  while IFS= read -r line; do
    ...
    if ! sync_core_is_selected "$id" "$desired_meta" "$actual_meta"; then
      continue
    fi
