#!/usr/bin/env bash
set -euo pipefail
DOTFILES_SCRIPT_LABEL="sync-terminal"
...
source_dotfiles_script "sync-adapters/terminal-lib.sh"
...
Options:
  ...
  --forget            Remove last-applied hash state (all managed profiles or one via --item)
  ...
  --item <name>       Restrict to one profile name
  --profiles-dir <path>
                      Profiles directory (default: <repo>/surfaces/terminal/desired)
...
plist="$real_plist"
sync_core_state_dir="${XDG_STATE_HOME:-$HOME/.local/state}/dotfiles/sync/terminal-app/profiles"
work_plist=""
profile_index=""
state_update_list=""
...
sync_cli_parse_script_option() {
  case "$1" in
  --profiles-dir)
    [[ $# -lt 2 ]] && die "missing value for --profiles-dir"
    profiles_dir="$2"
...
if [[ $sync_core_mode != "apply" && (-n $default_profile || -n $startup_profile) ]]; then
  die "--default-profile/--startup-profile are only valid with --apply"
fi
profiles_dir="$(sync_core_resolve_surface_dir "surfaces/terminal/desired" "$profiles_dir" "$default_profiles_dir")"
ROOT="$sync_core_root"
if [[ $sync_core_mode == "adopt" && $sync_core_in_place -eq 1 && ! -w $profiles_dir ]]; then
  die "profiles directory is not writable for --adopt --in-place: $profiles_dir (run from repo checkout or pass --profiles-dir)"
fi
...
build_profile_index_for_forget() {
  ...
  if [[ -n ${sync_core_item_filter:-} ]]; then
    printf '%s||\n' "$sync_core_item_filter" >"$profile_index"
