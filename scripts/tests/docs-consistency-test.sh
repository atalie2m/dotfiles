...
require_contains_file docs/commands.md '`EMACSDIR` overrides the writable Emacs config directory for `sync emacs`'
require_contains_file docs/commands.md '`DOTFILES_SYNC_VSCODE_BIN` overrides the `sync vscode` engine path.'
require_contains_file README.md 'scripts/tests/sync-emacs-smoke-test.sh'
...
require_contains_file docs/reconciled-surfaces.md 'Emacs config files converge fully to repo state on apply'
